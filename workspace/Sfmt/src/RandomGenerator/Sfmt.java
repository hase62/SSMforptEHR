...
public class Sfmt{
	    ...
	    protected int[]x=new int[352]; /* 状態テーブル */
	
	    protected int[]GetParity()
	    { return new int[]{ 0x00000001,0x00000000,0xe8148000,0xd0c7afa3 }; }
	
	    public String IdString() 
	    { return"SFMT-11213:68-14-3-7-3:effff7fb-ffffffef-dfdfbfff-7fffdbfd"; }
	
	    public void gen_rand_all() {
	        int a=0,b=272,c=344,d=348,y; int[]p=x;
	        
	        do {
	            y=p[a+3]^(p[a+3]<<24)^(p[a+2]>>>8)^((p[b+3]>>>7)&0x7fffdbfd);
	            p[a+3]=y^(p[c+3]>>>24)^(p[d+3]<<14);
	            y=p[a+2]^(p[a+2]<<24)^(p[a+1]>>>8)^((p[b+2]>>>7)&0xdfdfbfff);
	            p[a+2]=y^((p[c+2]>>>24)|(p[c+3]<<8))^(p[d+2]<<14);
	            y=p[a+1]^(p[a+1]<<24)^(p[a]>>>8)^((p[b+1]>>>7)&0xffffffef);
	            p[a+1]=y^((p[c+1]>>>24)|(p[c+2]<<8))^(p[d+1]<<14);
	            y=p[a]^(p[a]<<24)^((p[b]>>>7)&0xeffff7fb);
	            p[a]=y^((p[c]>>>24)|(p[c+1]<<8))^(p[d]<<14);
	            c=d; d=a; a+=4; b+=4;
	            if (b==352) b=0;
	        } while (a!=352);
	    }
	    ...
	    void period_certification() {
	        ...
	        index___=352; range=0; normal_sw=0; coin_bits=0; byte_pos=0;
	    ...
	    /*synchronized*/ public void InitMtEx(int[]init_key) {
	        ...
	        for (i=0;i<352;i++) x[i]=0x8b8b8b8b;
	        if (key_len+1>352) c=key_len+1; else c=352;
	        r=x[0]^x[172]^x[351]; r=(r^(r>>>27))*1664525;
	        x[172]+=r; r+=key_len; x[179]+=r; x[0]=r; c--;
	        for (i=1,j=0;j<c&&j<key_len;j++) {
	            r=x[i]^x[(i+172)%352]^x[(i+351)%352];
	            r=(r^(r>>>27))*1664525; x[(i+172)%352]+=r;
	            r+=init_key[j]+i; x[(i+179)%352]+=r;
	            x[i]=r; i=(i+1)%352;
	        }
	        for (;j<c;j++) {
	            r=x[i]^x[(i+172)%352]^x[(i+351)%352];
	            r=(r^(r>>>27))*1664525; x[(i+172)%352]+=r; r+=i;
	            x[(i+179)%352]+=r; x[i]=r; i=(i+1)%352;
	        }
	        for (j=0;j<352;j++) {
	            r=x[i]+x[(i+172)%352]+x[(i+351)%352];
	            r=(r^(r>>>27))*1566083941; x[(i+172)%352]^=r;
	            r-=i; x[(i+179)%352]^=r; x[i]=r; i=(i+1)%352;
	        }
	        period_certification();
	    }
	    ...
	    /*synchronized*/ public int NextMt() {
	        if (index___==352) { gen_rand_all(); index___=0; }
	        return x[index___++];
	    }
	    ...
	    /* 任意のモーメントの分布を発生 */
	    /*synchronized*/ public double NextSK(double v3, double v4)
	    {
	    	if(v4 < (7 + 4 * v3 * v3)/3) {
	    	...
	    	double l2 = (1/36)*(Math.sqrt((6*v4)-(8*v3*v3)-14)-2);
	    	double l1 = v3/(6*(1+6*l2));
	    	double k1 = (1-3*l2)/(1+(l1*l1)-(l2*l2));
	    	double k2 = l2/(1+(l1*l1)+(12*(l2*l2)));
	        if (normal_sw==0 && true) {
	            double t=Math.sqrt(-2*Math.log(1.0-NextUnif()));
	            double u=3.141592653589793*2*NextUnif();
	            normal_save=t*Math.sin(u); 
	            normal_sw=1;
	            double r = t*Math.cos(u);
	            if(v3==0 && v4==3) return r;
	            return (-l1)+(k1*r)+(l1*(r*r))+(k2*(r*r*r));
	        }else{ 
	        	normal_sw=0; 
	            if(v3==0 && v4==3) return normal_save;
	            return (-l1)+(k1*normal_save)+(l1*(normal_save*normal_save))
	            		+(k2*(normal_save*normal_save*normal_save));
	        }
	    }
